...
export class TipPrompt implements Prompt {
	...
	constructor(private _doc: SongDocument, type: string) {
		switch (type) {
			...
			case "selectionFunction": {
				this.message = div(
					...
					h3({ style: 'margin-top: 2rem;' }, "Variables"),
					...
					h3({ style: 'margin-top: 2rem;' }, "More variables"),
            ...
            case "operatorWaveform": {
                this.message = div(
                    ...
                    p('Waveform refers to the shape of the sound wave used by an individual FM wave.'),
                    p('By default the FM synth uses sine waves.'),
                );
            } break;
			case "filterType": {
				this.message = div(
			...
			case "loopControls":
			{
				this.message = div(h2("Loop Controls"), p("This enables the use of parameters that control how a chip wave should repeat."));
			}
			break;
				case "loopMode":
			{
				this.message = div(h2("Loop Mode"), p("This sets the way the chip wave loops when its ends are reached."), p("The \"Loop\" mode is the default: when the end of the loop is reached, it will jump back to the starting point of the loop."), p("The \"Ping-Pong\" mode starts playing the chip wave backwards when the end of the loop is reached. Once it reaches the start of the loop, it will start playing forwards again, endlessly going back and forth."), p("The \"Play Once\" mode stops the chip wave once the end is reached (or the start of the loop, if it's playing backwards)."), p("The \"Play Loop Once\" mode stops the chip wave once the end of the loop is reached (or the start of the loop, if it's playing backwards)."));
			}
			break;
			case "loopStart":
			{
				this.message = div(h2("Loop Start Point"), p("This specifies where the loop region of the chip wave starts. It's measured in \"samples\", or rather, it refers to a point on a waveform."), p("Be careful with tiny loop sizes (especially combined with high pitches), they may re-introduce aliasing even if the \"Aliasing\" checkbox is unchecked."));
			}
			break;
			case "loopEnd":
			{
				this.message = div(h2("Loop End Point"), p("This specifies where the loop region of the chip wave ends. It's measured in \"samples\", or rather, it refers to a point on a waveform."), p("The button next to the input box sets this to end of the chip wave."), p("Be careful with tiny loop sizes (especially combined with high pitches), they may re-introduce aliasing even if the \"Aliasing\" checkbox is unchecked."));
			}
			break;
				case "offset":
			{
				this.message = div(h2("Offset"), p("This specifies where the chip wave should start playing from. You can use this to chop up a large sample, to say, turn a drum loop into a drum kit! It's measured in \"samples\", or rather, it refers to a point on a waveform."));
			}
			break;
				case "backwards":
			{
				this.message = div(h2("Backwards"), p("When set, the chip wave will start playing backwards. After checking this, you may want to adjust the offset to start from a different point that makes sense for this mode."));
			}
			break;
			case "decimalOffset": {
				this.message = div(
		...
		this.container = div({class: "prompt", style: "width: 300px; max-height: 30rem; overflow-y: scroll;"},
			this.message, this._closeButton);
